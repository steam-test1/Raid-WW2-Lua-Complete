...
function ElementEnemyPreferedAdd:add()
	...
	if self._group_data.spawn_points then
		managers.groupai:state():add_preferred_spawn_points(self:_unique_string_id(), self._group_data.spawn_points)
	end

	if self._group_data.spawn_groups then
		managers.groupai:state():add_preferred_spawn_groups(self:_unique_string_id(), self._group_data.spawn_groups)
	end
end

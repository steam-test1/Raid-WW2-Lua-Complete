...
function ElementFleePoint:operation_add()
	if self._values.functionality == "loot_drop" then
		managers.groupai:state():add_enemy_loot_drop_point(self:_unique_string_id(), self._values.position)
	else
		managers.groupai:state():add_flee_point(self:_unique_string_id(), self._values.position, self._values.so_action)
	end
end
...
function ElementFleePoint:operation_remove()
	if self._values.functionality == "loot_drop" then
		managers.groupai:state():remove_enemy_loot_drop_point(self:_unique_string_id())
	else
		managers.groupai:state():remove_flee_point(self:_unique_string_id())
	end
end

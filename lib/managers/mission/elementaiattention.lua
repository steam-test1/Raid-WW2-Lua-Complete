...
function ElementAIAttention:_select_units_from_spawners()
	...
	for _, element_id in ipairs(self._values.instigator_ids) do
		local spawn_element = self:get_mission_element(element_id)
...
function ElementAIAttention:_fetch_unit_by_unit_id(unit_id)
	...
	if false then
		...
		unit = self._mission_script:worlddefinition():get_unit_on_load(tonumber(unit_id), callback(self, self, "_load_unit"))
	end

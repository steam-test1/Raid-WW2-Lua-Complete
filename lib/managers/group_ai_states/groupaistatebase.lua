...
function GroupAIStateBase:_execute_so(so_data, so_rooms, so_administered)
	...
	if closest_u_data then
		...
		if tweak_data.vehicle.AI_TELEPORT_DISTANCE < distance_to_so and objective_copy.objective_type == VehicleDrivingExt.SPECIAL_OBJECTIVE_TYPE_DRIVING and self:is_police_called() then
...
function GroupAIStateBase:spawn_one_criminal_ai(is_drop_in, char_name, spawn_on_unit)
	if not self:team_ai_enabled() or not self._ai_enabled or not managers.criminals:character_taken_by_name(char_name) and managers.criminals.MAX_NR_TEAM_AI <= managers.criminals:nr_AI_criminals() then
		return
	end

...
function SubtitleSequencePlayer:evaluate_at_time(time)
	if time ~= self._last_evaluated_time then
		...
		if subtitle ~= self.__previous_subtitle then
			self.__presenter:show_text(subtitle and subtitle:string() or "", subtitle and subtitle:duration(), subtitle and subtitle:color(), subtitle and subtitle:nationality_icon())
